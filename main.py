...
# 그라데이션 색상 계산 함수 추가
def calculate_gradient_color(event):
    # 현재 마우스 위치에 따라 RGB 값을 계산하여 그라데이션 색상 반환
    r = int((event.x / canvas.winfo_width()) * 255)
    g = int((event.y / canvas.winfo_height()) * 255)
    b = int(((event.x + event.y) / (canvas.winfo_width() + canvas.winfo_height())) * 255)
    return f'#{r:02x}{g:02x}{b:02x}'

# 그라데이션 색상으로 그림 그리는 함수 추가
def paint_gradient(event):
    global last_x, last_y
    if last_x is not None and last_y is not None:
        canvas.create_line(last_x, last_y, event.x, event.y, fill=calculate_gradient_color(event), width=2)
    last_x, last_y = event.x, event.y
...
# 랜덤 색상으로 브러쉬 색상 설정 함수 추가
def set_random_brush_color():
    global brush_color
    brush_color = get_random_color()

def open_image():
    file_path = filedialog.askopenfilename(filetypes=[("Image files", "*.jpg;*.jpeg;*.png;*.gif")])  # 이미지 파일을 선택합니다.
    if file_path:  # 파일이 선택되었다면
        image = PhotoImage(file=file_path)  # 이미지 파일을 로드합니다.
        canvas.image = image  # 이미지를 캔버스에 저장합니다.
        canvas.create_image(0, 0, anchor="nw", image=image)  # 캔버스에 이미지를 표시합니다.
...
# 랜덤 색상 선택 버튼 생성
button_random_color = Button(window, text="Random Brush Color", command=set_random_brush_color)
button_random_color.pack(side=LEFT)

# 그라데이션 모드로 변경하는 함수 추가
def set_paint_mode_gradient():
    canvas.bind("<B1-Motion>", paint_gradient)

# 그라데이션 모드로 전환하는 버튼 생성
button_gradient_mode = Button(window, text="Gradient Mode", command=set_paint_mode_gradient)
button_gradient_mode.pack(side=RIGHT)

button_open_image = Button(window, text="이미지 열기", command=open_image)  # 이미지 열기 버튼 생성
button_open_image.pack(side=LEFT)  # 이미지 열기 버튼을 윈도우에 배치
