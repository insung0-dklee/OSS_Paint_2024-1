...
def update_timer():
    ...
    window.after(1000, update_timer)  # 1초마다 update_time 함수를 호출
...
"""
그림 그리는 것을 획 단위로 그리도록 개선, 획 단위로 지우는 지우개 기능 추가, 지웠던 획을 다시 되돌리는 기능 추가
그림 그리는 것을 획 단위로 그리도록 개선하였으며, 마우스 클릭 후 놓을 때까지를 한 획으로 보았다.
