...
def bind_shortcuts():
    ...
    window.bind("<Control-z>", lambda event: erase_last_stroke())
...
# 선택한 도형을 저장할 변수와 선택한 도형의 초기 좌표를 저장할 변수
selected_shape = None
start_x, start_y = None, None
...
def select_shape(event):
    ...
    if item:
        selected_shape = item[0]
        start_x, start_y = event.x, event.y
        canvas.itemconfig(selected_shape, width=3)

def deselect_shape(event):
    global selected_shape
    if selected_shape:
        canvas.itemconfig(selected_shape, width=1)
        selected_shape = None

def move_shape(event):
    global start_x, start_y, selected_shape
    if selected_shape:
        dx = event.x - start_x
        dy = event.y - start_y
...
def resize_shape(event):
    global selected_shape, start_x, start_y
    if selected_shape:
        x1, y1, x2, y2 = canvas.coords(selected_shape)
        dx = event.x - start_x
        dy = event.y - start_y
        new_x2 = x2 + dx
        new_y2 = y2 + dy
        canvas.coords(selected_shape, x1, y1, new_x2, new_y2)
        start_x, start_y = event.x, event.y

def change_shape_color(event):
    global selected_shape
    if selected_shape:
        new_color = askcolor()[1]
        if new_color:
            canvas.itemconfig(selected_shape, fill=new_color)

def delete_shape(event):
    global selected_shape
    if selected_shape:
        canvas.delete(selected_shape)
        selected_shape = None

canvas.bind("<Button-1>", select_shape)
canvas.bind("<ButtonRelease-1>", deselect_shape)
canvas.bind("<B1-Motion>", move_shape)
window.bind("<Delete>", delete_shape)
window.bind("<c>", change_shape_color)
