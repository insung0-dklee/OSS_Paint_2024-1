...
erase_size : 현 지우개의 크기 값을 나타냅니다.
'plus' and event.state & 0x4 : 키보드의 +와 좌측 ctrl 키를 동시에 입력 받을 시
'minus' and event.state & 0x4 : 키보드의 -와 좌측 ctrl 키를 동시에 입력 받을 시
Max_erase_size : 지우개 크기의 최댓값
Min_erase_size : 지우개 크기의 최솟값

"""
def change_erase_size(event):
    global erase_size
...
def erase(event):
    ...
    x1, y1 = (event.x - erase_size), (event.y - erase_size)
    x2, y2 = (event.x + erase_size), (event.y + erase_size)
...
canvas.bind("<KeyPress>", change_erase_size)
canvas.focus_set()
