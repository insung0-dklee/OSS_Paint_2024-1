...
"""
격지선 함수 추가
canvas_width : 현재 캔버스의 너비를 받음
canvas_height : 현재 캔버스의 높이를 받음

draw_grid : x의 좌표 10부터 간격을 10으로 하는 회색 격자선 생성
fill : 격자선 색 지정
tags : 격자선의 일괄 제거를 위해 같은 태그로 묶어둠

grid_switch : 격자선이 없다면 생성, 있다면 삭제를 시키는 함수
grid_on : 격자선의 유무를 확인하는 전역 변수
격자선이 켜져 있다면 tags="grid"를 삭제, 아니라면 draw_grid 함수 호출
grid_on = not grid_on : grid_on 변수를 True <-> False로 변경시키는 문장
"""
...
def draw_grid():
    ...
    # 가로 격자선
    for i in range(10, canvas_width, 10):
        canvas.create_line(i, 0, i, canvas_height, fill="gray", tags="grid")

    # 세로 격자선
    for i in range(10, canvas_height, 10):
        canvas.create_line(0, i, canvas_width, i, fill="gray", tags="grid")
...
button_new_window = Button(window, text="새 창 열기",
                           command=create_new_window)  # "새 창 열기"라는 버튼 생성 command: 버튼 클릭 시 create_new_window: 새로운 창을 만듦
...
grid_on = False
button_grid = Button(window, text="Grid", command=grid_switch)
