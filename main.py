...
def apply_light_mode(): # 라이트 모드 적용(기본)
    window.config(background="sky blue") # 윈도우 배경색
    canvas.config(background="white") # 캔버스 배경색
    button_frame.config(background="sky blue") # 버튼 프레임 배경색
    for widget in button_frame.winfo_children(): 
        widget.config(background="light grey", foreground="black") # 버튼 프레임 안의 모든 버튼들 배경색, 글자색
    timer_label.config(background="white", foreground="black") # 타이머 라벨 배경색, 글자색

def apply_dark_mode(): # 다크 모드 적용
    window.config(background="grey20") # 윈도우 배경색
    canvas.config(background="grey30") # 캔버스 배경색
    button_frame.config(background="grey20") # 버튼 프레임 배경색
    for widget in button_frame.winfo_children():
        widget.config(background="grey40", foreground="white") # 버튼 프레임 안의 모든 버튼들 배경색, 글자색
    timer_label.config(background="grey20", foreground="white") # 타이머 라벨 배경색, 글자색
