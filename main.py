...
def create_line_stright(event=None):
    select_shape_color()
    canvas.bind("<Button-1>", start_line_stright)

def start_line_stright(event): # 그리려는 선의 시작점을 잡아주고 그리기를 시작하는 기능
    ...
    if is_shift_pressed:  # 쉬프트 키가 눌려 있는지 확인
        canvas.bind("<B1-Motion>", lambda event: draw_line_stright_45_degree(event)) #선을 45도 각도로 그림
    else:
        canvas.bind("<B1-Motion>", lambda event: draw_line_stright(event)) #선을 그림
    canvas.bind("<ButtonRelease-1>", finish_line_stright) # 마우스 버튼을 떼면 선 그리기 종료

def draw_line_stright(event): # 선을 그리는 기능
    global start_x, start_y, current_shape
    canvas.delete("temp_shape")
    current_shape = canvas.create_line(start_x, start_y, event.x, event.y, fill=shape_outline_color, tags="temp_shape")

def draw_line_stright_45_degree(event): # 선을 45도 각도로 그리는 기능
    global start_x, start_y, current_shape
    canvas.delete("temp_shape")
...
def finish_line_stright(event): # 선 그리기를 마치는 기능
...
def choose_shape(event):
    popup = Menu(window, tearoff=0)
    popup.add_command(label="Line", command=lambda: create_line_stright(event))
