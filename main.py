...
def draw_speech_bubble(event):
    ...
    # 말풍선 본체 (타원)
    canvas.create_oval(x, y, x + bubble_width, y + bubble_height, fill="white", outline="black")
    
    # 말풍선 포인터 (삼각형)
    points = [
        x + bubble_width // 2 - pointer_size, y + bubble_height,
        x + bubble_width // 2 + pointer_size, y + bubble_height,
        x + bubble_width // 2, y + bubble_height + pointer_size
    ]
...
def set_shape_mode(mode):
    ...
    selected_shape = mode
    if selected_shape == "speech_bubble":
        canvas.bind("<Button-1>", draw_speech_bubble)
...
button_speech_bubble = Button(window, text="Speech Bubble", command=lambda: set_shape_mode("speech_bubble"))
button_speech_bubble.pack(side=LEFT)
