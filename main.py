...
# 흑백 필터를 적용하기 전에 개체의 색상을 저장하는 딕셔너리
original_colors = {}

def is_valid_hex_color(color):
    """
    RGB 색상 코드가 유효한지 확인하는 함수이다.
    
    Parameters:
        color (str): RGB 색상 코드
        
    Returns:
        bool: 색상 코드의 유효성 여부 (True 또는 False)
    """
    # 색상 코드가 7자리(포함된 '#' 제외)인지 확인한다.
    if len(color) != 7:
        return False
    try:
        # 색상 코드를 16진수로 변환하여 유효성을 확인한다.
        int(color[1:], 16)
    except ValueError:
        return False
    return True
...
def apply_grayscale():
    ...
    # 캔버스에 그려진 모든 개체를 가져온다.
    objects = canvas.find_all()
    for obj in objects:
        # 개체가 선 또는 원인 경우에만 처리한다.
        if canvas.type(obj) == "line" or canvas.type(obj) == "oval":
            # 현재 개체의 색상을 가져옵니다.
            original_colors[obj] = canvas.itemcget(obj, "fill")

            # RGB 색상을 가져온다.
            fill_color = canvas.itemcget(obj, "fill")
            ...
            # 흑백으로 색상을 변경한다.
            canvas.itemconfig(obj, fill=gray_hex)

def reset_color():
    # 흑백 필터 적용 이전의 색상으로 되돌리는 함수이다.

    global original_colors
    # 저장된 원래 색상을 개체에 적용한다.
    for obj, color in original_colors.items():
        canvas.itemconfig(obj, fill=color)
    # 원래 색상을 저장한 딕셔너리를 초기화한한다.
    original_colors = {}
...
# 흑백 필터 적용 버튼
button_apply_grayscale = Button(window, text="Apply Grayscale Filter", command=apply_grayscale)
button_apply_grayscale.pack(side=LEFT)

# 원래 색상으로 되돌리는 버튼
button_reset_color = Button(window, text="Reset Color", command=reset_color)
button_reset_color.pack(side=LEFT)
