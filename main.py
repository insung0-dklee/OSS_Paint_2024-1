...
# 4각별을 생성하는 함수       
def create_four_pointed_star(event=None):
    select_shape_color()
    canvas.bind("<Button-1>", start_four_pointed_star)

# 4각별 모양 그릴 위치 정하고 생성하는 함수 호출
def start_four_pointed_star(event):
    global start_x, start_y, current_shape
    start_x, start_y = event.x, event.y
    current_shape = None
    canvas.bind("<B1-Motion>", lambda event: draw_four_pointed_star(event))
    canvas.bind("<ButtonRelease-1>", finish_four_pointed_star)

# 4각별 모양 생성하기
def draw_four_pointed_star(event):
    global start_x, start_y, current_shape
    canvas.delete("temp_shape")
    outer_radius = ((start_x - event.x)**2 + (start_y - event.y)**2)**0.5
    ...
    for i in range(4):
        angle_outer = math.radians(i * 90 - 90)
        angle_inner = math.radians(i * 90 + 45 - 90)
...
# 4각별 모양 그리기 종료
def finish_four_pointed_star(event):
    global current_shape
    canvas.unbind("<B1-Motion>")  
    canvas.unbind("<ButtonRelease-1>")
    if current_shape:
...
def choose_shape(event):
    ...
    popup.add_command(label="Four Pointed Star", command=lambda: create_four_pointed_star(event))
