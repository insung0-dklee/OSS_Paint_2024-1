...
from tkinter import filedialog, PhotoImage  # 파일 대화 상자 및 사진 모듈
...
def paint_airbrush(event, canvas):
    for _ in range(dot_count.get()):  # 에어브러쉬 효과를 위해 여러 개의 작은 점을 그림
        ...
        current_stroke.append((x - radius, y - radius, x + radius, y + radius, brush_color))
...
def start_paint_pressure(event):
    global start_time, last_x, last_y, current_stroke
...
def paint_pressure(event, canvas):
    ...
    current_stroke.append((x1, y1, x2, y2, brush_color))
    last_x, last_y = event.x, event.y
...
def paint(event, canvas):
    ...
    if last_x and last_y:
        ...
        current_stroke.append((last_x, last_y, event.x, event.y, brush_color, brush_size))
    last_x, last_y = event.x, event.y
...
def set_brush_mode(canvas, mode):  # 브러쉬 모드를 변경하는 함수
    ...
    if brush_mode == "solid":  # 브러쉬 모드가 solid면
        set_paint_mode_normal(canvas)  # 실선(기본) 브러쉬로 변경
...
def reset_brush(canvas):
    ...
    set_paint_mode_normal(canvas)  # 브러시 모드 초기화
...
def paint_stroke(event):  # 획 그림
    ...
    current_stroke.append((x1, y1, x2, y2, brush_color, brush_size))
    x1, y1 = x2, y2
...
def erase_last_stroke():  # 마지막으로 그린 획을 지움
    if strokes:
        ...
        for line in last_stroke:
            if len(line) == 5:  # 점을 지울 때
                x1, y1, x2, y2, color = line
                canvas.create_oval(x1, y1, x2, y2, fill="white", outline="white")
            else:
                x1, y1, x2, y2, color, size = line
                canvas.create_line(x1, y1, x2, y2, fill="white", width=size)
